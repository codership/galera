...
MYSQL_BASE_DIR=${MYSQL_BASE_DIR:-"/usr/local"}
...
MYSQL_DATA_DIR=${MYSQL_DATA_DIR:-"/var/db/mysql"}
...
#    fi
    ...
    WSREP=${WSREP:-"/usr/local/lib/galera/libgalera_smm.so"}
...
[ "$OS" == "FreeBSD" ] && LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/lib/gcc44
[ "$OS" == "Darwin"  ] && export -n LD_LIBRARY_PATH
