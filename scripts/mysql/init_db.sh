...
chmod go-r "$init_sql"
echo "ALTER USER 'root'@'localhost' IDENTIFIED WITH caching_sha2_password BY '$root_pswd';" >> "$init_sql"
echo "UPDATE mysql.user SET Host = '%' WHERE User = 'root';" >> "$init_sql"
echo "CREATE USER IF NOT EXISTS 'test'@'%' IDENTIFIED WITH caching_sha2_password BY '$test_pswd';" >> "$init_sql"
