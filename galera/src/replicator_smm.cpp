                                                     ...
                                                     void* cc_buf)
{
    ...
    if (group_seqno <= sst_seqno_)
    {
        log_info << "####### skipping CC " << group_seqno << ", local";
    ...
    // 2) in-order when no state transfer
    ...
    // If the state transfer was requested, the CC was contained in
    // either SST or IST.
