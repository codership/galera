...
gt_group::perform_state_exchange()
{
    ...
    for (i = 0; i < nodes_num; i++)
    {
        ...
        for (j = 0; j < nodes_num; j++) {
            gcs_group_state_t ret =
                gcs_group_handle_state_msg (&(nodes[j]->group),
                                            &state_msg);
      ...
      id()
{
    if (name)
    {
        ...
        snprintf(addr_str, str_len - 1, "name:%s", id);
        snprintf(addr_str, str_len - 1, "addr:%s", id);

        gcs_group_init(&group, NULL, name_str, addr_str, 0, 0, 0);
     }
}
...
gt_group::gt_group(int const num, bool const prim)
    : nodes(),
      nodes_num(0),
      primary(prim)
{
    if (num > 0)
    {
        for (int i = 0; i < num; ++i)
        {
            char name[32];
            sprintf(name, "%d", i);
            add_node(new gt_node(name), true);
