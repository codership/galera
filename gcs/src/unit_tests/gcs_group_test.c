...
START_TEST(test_gcs_group_find_donor)
{
    ...
    for(int i = 0; i < number; i++)
    {
        ...
        gcs_node_init(&nodes[i], NULL, name, name, "", 0, 0, 0, i > joiner ? 1 : 0);
        nodes[i].status = GCS_NODE_STATE_SYNCED;
        nodes[i].state_msg = gcs_state_msg_create(&empty_uuid, &empty_uuid, &empty_uuid,
                                                  0, 0, seqnos[i], 0,
...
#define SARGS(s) s, strlen(s)
    //========== sst ==========
    donor = gcs_group_find_donor(&group, joiner, SARGS("home3"), &empty_uuid, GCS_SEQNO_ILL);
    fail_if(donor != -EHOSTDOWN);

    donor = gcs_group_find_donor(&group, joiner, SARGS("home1,home2"), &empty_uuid, GCS_SEQNO_ILL);
    fail_if(donor != 1);
    ...
    // ========== ist ==========
    // by name.
    donor = gcs_group_find_donor(&group, joiner, SARGS("home0,home1,home2"), group_uuid, ist_seqno);
    fail_if(donor != 1);

    group.quorum.act_id = 200; // in safe range. in segment.
    donor = gcs_group_find_donor(&group, joiner, SARGS("home2"), group_uuid, ist_seqno);
    fail_if(donor != 1);

    group.quorum.act_id = 200; // in safe range. cross segment.
