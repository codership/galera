...
START_TEST(test_trac_762)
{
    ...
    // handles install in trans view and marks nodes 1 and 2 to have un state
    {
        ...
        tr1.add_member(tp1->uuid());
        tr1.add_member(tp2->uuid());
        pc1->handle_view(tr1);
        pc2->handle_view(tr1);

        View tr2(ViewId(V_TRANS, tp3->uuid(), view_seq));
        tr2.add_member(tp3->uuid());
        pc3->handle_view(tr2);

        ++view_seq;
        View reg(ViewId(V_REG, tp1->uuid(), view_seq));
        reg.add_member(tp1->uuid());
        reg.add_member(tp2->uuid());
        reg.add_member(tp3->uuid());
        ...
        tr3.add_member(tp1->uuid());
        tr3.add_member(tp2->uuid());
        tr3.add_partitioned(tp3->uuid());

        pc1->handle_view(tr3);
        pc2->handle_view(tr3);

        View tr4(ViewId(V_TRANS, tp1->uuid(), view_seq));
        tr4.add_member(tp3->uuid());
        tr4.add_partitioned(tp1->uuid());
        tr4.add_partitioned(tp2->uuid());
        pc3->handle_view(tr4);
        pc3->handle_up(0, *dg, ProtoUpMeta(tp1->uuid()));
        delete dg;
    }
    ...
    // 1 and 2
    {
        ...
        tr.add_member(tp1->uuid());
        tr.add_member(tp2->uuid());
        pc1->handle_view(tr);
        pc2->handle_view(tr);
